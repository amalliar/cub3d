...
/*   Updated: 2020/09/27 12:12:03 by amalliar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				init_door_segments(t_player_data *pd)
{
	if (ft_strchr(NS, pd->door->type))
	{
		...
		init_fpoint(&(pd->s1).p1, pd->map_x, pd->map_y + 1);
		init_fpoint(&(pd->s2).p0, pd->map_x + 1, pd->map_y);
		init_fpoint(&(pd->s2).p1, pd->map_x + 1, pd->map_y + 1);
		init_fpoint(&(pd->s3).p0, pd->map_x + 1 - pd->door->s_timer, \
			pd->map_y + 0.5);
		init_fpoint(&(pd->s3).p1, pd->map_x + 1, pd->map_y + 0.5);
	}
	else
	{
		init_fpoint(&(pd->s1).p0, pd->map_x, pd->map_y);
		init_fpoint(&(pd->s1).p1, pd->map_x + 1, pd->map_y);
		init_fpoint(&(pd->s2).p0, pd->map_x, pd->map_y + 1);
		init_fpoint(&(pd->s2).p1, pd->map_x + 1, pd->map_y + 1);
		init_fpoint(&(pd->s3).p0, pd->map_x + 0.5, \
			pd->map_y + 1 - pd->door->s_timer);
		init_fpoint(&(pd->s3).p1, pd->map_x + 0.5, pd->map_y + 1);
	}
...
ft_strchr(EW, pd->door->type) && r.x >= (pd->s2).p0.x && r.x <= (pd->s2).p1.x)))
	...
	if (!pd->door_hit)
		return (false);
	pd->map_x = (pd->ray_dir_x > 0) ? r.x : r.x - 1;
	pd->map_y = (pd->ray_dir_y > 0) ? r.y : r.y - 1;
							...
							double mx, double my)
{
	...
	{
		init_fpoint(&(pd->s1).p0, mx, my + 1 - pd->secret->s_timer);
		init_fpoint(&(pd->s1).p1, mx + 1, my + 1 - pd->secret->s_timer);
	}
	else if (ft_strchr(MP_SSECRETS, pd->secret->type))
	{
		init_fpoint(&(pd->s1).p0, mx, my + pd->secret->s_timer);
		init_fpoint(&(pd->s1).p1, mx + 1, my + pd->secret->s_timer);
	}
	else if (ft_strchr(MP_ESECRETS, pd->secret->type))
	{
		init_fpoint(&(pd->s1).p0, mx + pd->secret->s_timer, my);
		init_fpoint(&(pd->s1).p1, mx + pd->secret->s_timer, my + 1);
	}
	else if (ft_strchr(MP_WSECRETS, pd->secret->type))
	{
		init_fpoint(&(pd->s1).p0, mx + 1 - pd->secret->s_timer, my);
		init_fpoint(&(pd->s1).p1, mx + 1 - pd->secret->s_timer, my + 1);
	}
	init_fpoint(&(pd->ray).p0, pd->pos_x, pd->pos_y);
	init_fpoint(&(pd->ray).p1, pd->pos_x + pd->ray_dir_x, \
		pd->pos_y + pd->ray_dir_y);
...
static bool				secret_hit(t_player_data *pd)
{
	...
	{
		pd->secret_hit = 1;
		pd->map_x = (pd->ray_dir_x > 0) ? res.x : res.x - 1;
		pd->map_y = (pd->ray_dir_y > 0) ? res.y : res.y - 1;
...
void					extend_ray(t_scene *scene)
{
	...
	else if (ft_strchr(MP_SECRETS, (md->map)[(int)pd->map_y][(int)pd->map_x]))
	{
		...
		init_secret_segments(pd, pd->map_x, pd->map_y);
		if (secret_hit(pd))
			return ;
	}
	pd->hit = 0;
